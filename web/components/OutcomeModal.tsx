...
export function OutcomeModal({
    ...
    useEffect(() => {
        ...
        if ((isWinner || isSpectator) && payout > 0n) {
    ...
    }, [isOpen, isWinner, isSpectator, payout]);

    if (!isOpen) return null;

    const netProfit = payout > stake ? payout - stake : 0n;
    const resolvedWinner = winnerAddress || opponentAddress;
    ...
    const handleShare = () => {
        ...
        if (typeof window !== 'undefined') {
            const text = isWinner
                ...
                : isSpectator
                ? `⚡ Duel #${duelId} on @WagrApp just settled! Winner: ${shortAddr(resolvedWinner)} took home ${fmtUsd(payout)} USDso. Check it out: ${window.location.origin}/duel/${duelId}`
            ...
            <div
                className={`relative w-full max-w-lg rounded-3xl p-6 sm:p-8 overflow-hidden shadow-2xl transition-all animate-in zoom-in-95 duration-300 border ${
                    isWinner
                        ...
                        : isSpectator
                        ? 'border-cyan-500/50 bg-gradient-to-b from-[#0a1e28] via-bg-elevated to-bg ring-4 ring-cyan-500/20 shadow-[0_0_90px_rgba(6,182,212,0.25)]'
                ...
                <div className="relative z-10 text-center space-y-6">
                    ...
                    <div className="relative mx-auto flex items-center justify-center">
                        {isWinner ? (
                            ...
                            <div className="relative">
                                <div className="absolute inset-0 -m-4 rounded-full bg-cyan-400/20 blur-md animate-pulse" />
                                <div className="h-20 w-20 sm:h-24 sm:w-24 rounded-3xl bg-gradient-to-tr from-cyan-400 via-brand to-teal-300 p-0.5 shadow-brand-glow flex items-center justify-center">
                                    <div className="h-full w-full rounded-[22px] bg-bg flex items-center justify-center">
                                        <TrophyIcon className="w-10 h-10 sm:w-12 sm:h-12 text-cyan-400 drop-shadow-[0_0_15px_rgba(34,211,238,0.6)]" />
                                    </div>
                                </div>
                            </div>
                        ) : (
                    ...
                    <div className="space-y-1.5">
                        <div
                            className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest border ${
                                isWinner
                                    ...
                                    : isSpectator
                                    ? 'border-cyan-500/40 bg-cyan-500/10 text-cyan-400 shadow-[0_0_20px_rgba(6,182,212,0.3)]'
                        ...
                        >
                            {isWinner
                                ...
                                : isSpectator
                                ? isSettled
                                    ? '⚖️ MATCH SETTLED ON SHANNON'
                                    : '⚡ ORACLE RESOLUTION CONFIRMED'
                                : isSettled
                                ? '💀 MATCH SETTLED'
                                : '⚡ ORACLE RESOLVED'}
                        </div>
                        ...
                        <h2 className="text-2xl sm:text-3xl font-black text-white tracking-tight uppercase">
                            {isWinner
                                ? isSettled
                                    ...
                                    : 'Oracle Resolved: You Won!'
                                : isSpectator
                                ? `Winner: ${shortAddr(resolvedWinner)}`
                                : 'Defeated This Round'}
                        </h2>
                        ...
                        <p className="text-xs sm:text-sm text-slate-300 max-w-sm mx-auto leading-relaxed">
                            {isWinner
                                ...
                                : isSpectator
                                ? `The DreamDEX prediction window closed and settled on Shannon. ${shortAddr(resolvedWinner)} secured the round with zero-click payout!`
                                : 'The DreamDEX window swung against your side this time. The next price window is already forming.'}
                        </p>
                    </div>

                    {/* Giant Payout / Pot Display */}
                    {isWinner ? (
                        ...
                        <div className="rounded-2xl bg-black/40 border border-cyan-500/30 p-5 space-y-1">
                            <span className="text-[11px] font-bold uppercase tracking-wider text-cyan-400/80">
                                Total Pot Paid Out
                            </span>
                            <div className="font-mono text-3xl sm:text-4xl font-extrabold text-cyan-300 drop-shadow-[0_0_25px_rgba(34,211,238,0.5)]">
                                {counter > 0 ? counter.toFixed(2) : fmtUsd(payout)} USDso
                            </div>
                            <div className="text-[11px] text-slate-300 flex items-center justify-center gap-2 pt-1 font-mono">
                                <span>Winner: <strong className="text-white">{shortAddr(resolvedWinner)}</strong></span>
                                <span>•</span>
                        ...
                        {isWinner ? (
                            ...
                            <>
                                <Link
                                    ...
                                    className="w-full py-4 rounded-2xl bg-gradient-to-r from-cyan-400 via-brand to-teal-400 hover:from-cyan-500 hover:to-teal-500 font-extrabold text-xs sm:text-sm text-black shadow-brand-glow transition-all flex items-center justify-center gap-2 uppercase tracking-wider"
                                >
                                    <ZapIcon className="w-4 h-4" />
                                    <span>Create Your Own Duel</span>
                                </Link>
